...
| Событие | Описание |
|---|---|
| `custom_replace` | Применена явная замена, заданная пользователем через `custom_replacements` |
| `parent_resolved` | Регион-родитель переопределён на вариант «без АО», т.к. в данных найдены автономные округа отдельно (напр. Архангельская область → без НАО) |
...
### 5.1 Явная замена, если автоматика не сработала

Для Тюменской и Архангельской областей нормализатор автоматически выбирает вариант «с АО» или «без АО» по контексту датасета (см. события `parent_resolved` / `parent_kept`). Если результат не устраивает — например, в данных нет округов как отдельных строк, но вы точно знаете, что данные без НАО — используйте параметр `custom_replacements`.

`custom_replacements` принимает словарь `{исходное_значение: эталонное_название}`. **Ключ** — точная строка из вашей колонки, **значение** — точное название из [эталонного справочника](https://github.com/tochno-st/reg_normalizer/blob/main/data/interim/regions_etalon_v2.0.yaml). Если указанного названия в справочнике нет, будет выброшена ошибка `ValueError`.
...
result = matcher.match_dataframe(
    ...
    'region_name',
    custom_replacements={
        # Исходное значение из колонки → точное название из эталона
...
Явные замены применяются **после** автоматического разрешения и имеют наивысший приоритет. В журнале `get_match()` такие строки отображаются с событием `custom_replace` и идут первыми.

Актуальные названия для Тюменской и Архангельской областей:

| Вариант | Название в эталоне |
|---|---|
| Тюменская с ХМАО и ЯНАО | `Тюменская область (с автономными округами)` |
| Тюменская без ХМАО и ЯНАО | `Тюменская область (без автономных округов)` |
| Архангельская с НАО | `Архангельская область (с автономным округом)` |
| Архангельская без НАО | `Архангельская область (без автономного округа)` |

Параметр работает одинаково в `match_dataframe` и `attach_fields`.
